...
def pick_random_song(artist_discographies):

    artist = random.choice(list(artist_discographies.keys()))

    album = random.choice(list(artist_discographies[artist].keys()))

    song = random.choice(artist_discographies[artist][album])

    return f"Artist: {artist}, Album: {album}, Song: {song}"

print(pick_random_song(artist_discographies))